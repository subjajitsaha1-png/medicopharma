...
import {
  ...
  OpioidSignalingDiagram,
  ParkinsonDiagram,
  MigraineDiagram,
  AlcoholMetabolismDiagram,
  OrganophosphateDiagram,
  NaloxoneReversalDiagram,
  VitaminKCycleDiagram,
  HistamineAllergyDiagram,
  SerotoninSyndromeDiagram,
  UterineMuscleDiagram,
  AntitubercularDiagram,
  AntifungalDiagram,
  AntimalarialDiagram,
  AnthelminticDiagram,
  CheckpointBlockadeDiagram,
  PgpEffluxDiagram,
  IonTrappingDiagram,
  BetaBlockerDiagram,
  CalciumChannelBlockerDiagram,
  NitrateDiagram,
  StatinDiagram,
  AntiemeticDiagram,
  LaxativeDiagram,
  OpioidToleranceDiagram,
  GoutDiagram,
  CalcineurinInhibitorDiagram,
  AcyclovirMechanismDiagram,
  NeuraminidaseInhibitorDiagram,
  EpinephrineAnaphylaxisDiagram,
  Pde5InhibitorDiagram,
  AntitussiveDiagram,
  MuscleRelaxantDiagram,
  AnxiolyticComparisonDiagram,
  IronAbsorptionDiagram,
  B12FolateDiagram,
  HitDiagram,
  MetabolismPhasesDiagram,
  BbbPermeabilityDiagram,
...
const DIAGRAMS: { topicId: string; title: string; description: string; pearl: string; Component: () => ReactNode }[] = [
  ...
  { topicId: "general", title: "P-Glycoprotein Efflux Pump", description: "How this ABC transporter limits drug absorption and CNS entry, and why P-gp inhibitors/inducers cause major drug interactions.", pearl: "Digoxin is the classic P-gp substrate — this is exactly why verapamil and other P-gp inhibitors can precipitate digoxin toxicity.", Component: PgpEffluxDiagram },
  { topicId: "general", title: "Renal Tubular Ion Trapping", description: "How urine pH determines whether a weak acid or weak base gets reabsorbed or trapped for excretion.", pearl: "Urinary alkalinization in aspirin overdose works by trapping the ionized (charged) form of the weak acid in the tubule, where it can't be reabsorbed.", Component: IonTrappingDiagram },
  { topicId: "general", title: "Phase I vs Phase II Drug Metabolism", description: "Oxidation/reduction/hydrolysis (mainly CYP450) vs conjugation reactions, with a prodrug-activation example.", pearl: "Some drugs (e.g. lorazepam) skip Phase I entirely — useful in liver disease, since Phase II capacity is often preserved longer than Phase I.", Component: MetabolismPhasesDiagram },
  { topicId: "general", title: "Blood-Brain Barrier Drug Permeability", description: "Tight junctions, lipophilicity, and P-glycoprotein efflux together determining CNS drug entry.", pearl: "Inflammation (e.g. meningitis) disrupts the BBB, letting normally-excluded antibiotics like penicillin penetrate far better — a key reason meningitis dosing differs from routine dosing.", Component: BbbPermeabilityDiagram },
  ...
  { topicId: "ans", title: "Neuromuscular Blockade: Depolarizing vs Non-Depolarizing", description: "Succinylcholine vs vecuronium/atracurium at the same Nm receptor — two opposite mechanisms of muscle paralysis.", pearl: "Neostigmine reverses a non-depolarizing block but WORSENS a depolarizing (succinylcholine) block — a classic exam trap testing whether you know which mechanism you're dealing with.", Component: NmjBlockadeDiagram },
  { topicId: "ans", title: "Epinephrine in Anaphylaxis", description: "Combined alpha-1, beta-1, and beta-2 actions shown on one diagram — why epinephrine alone reverses the whole reaction.", pearl: "Epinephrine is first-line and given BEFORE antihistamines or steroids — those don't act fast enough for the acute, life-threatening reaction.", Component: EpinephrineAnaphylaxisDiagram },
  { topicId: "cvs", title: "RAAS Cascade & Drug Targets", description: "Direct renin inhibitor, ACE inhibitor, and ARB sites of action marked directly on the cascade, from renin release through to vasoconstriction and aldosterone release.", pearl: "Direct renin inhibitors (aliskiren) act one step earlier than ACE inhibitors — blocking the very first step of the whole cascade.", Component: RaasDiagram },
  ...
  { topicId: "cvs", title: "Diuretic Sites Along the Nephron", description: "Loop, thiazide, potassium-sparing, carbonic anhydrase inhibitor, and osmotic diuretics, each marked at their exact nephron segment.", pearl: "Loop diuretics act at the thick ascending limb, where up to 25% of filtered Na+ is normally reabsorbed — that's why they're the most potent diuretic class.", Component: NephronDiureticDiagram },
  { topicId: "cvs", title: "Vitamin K Cycle & Warfarin", description: "How warfarin blocks vitamin K epoxide reductase, halting the carboxylation of clotting factors II, VII, IX, and X.", pearl: "Warfarin's effect is delayed 2-3 days because it only blocks new factor synthesis — existing factors have to degrade first, which is also why vitamin K reversal isn't instant either.", Component: VitaminKCycleDiagram },
  { topicId: "cvs", title: "Beta-Blocker Cardiac Mechanism", description: "Beta-1 blockade's effect on rate, contractility, and AV conduction — cardioselective vs non-selective.", pearl: "Non-selective agents also block beta-2 — bronchospasm risk in asthma, and masking of tachycardia (a key hypoglycemia warning sign) in diabetics.", Component: BetaBlockerDiagram },
  { topicId: "cvs", title: "Calcium Channel Blocker Mechanism", description: "L-type channel block in vascular smooth muscle vs cardiac tissue — dihydropyridine vs non-dihydropyridine selectivity.", pearl: "Combining a non-dihydropyridine (verapamil, diltiazem) with a beta-blocker risks severe bradycardia or heart block — dihydropyridines are the safer pairing.", Component: CalciumChannelBlockerDiagram },
  { topicId: "cvs", title: "Nitrate Mechanism", description: "The nitric oxide/cGMP pathway causing venodilation and reduced preload.", pearl: "Nitrates and PDE5 inhibitors (sildenafil) are an absolute contraindication together — both raise cGMP through different mechanisms, risking dangerous hypotension.", Component: NitrateDiagram },
  { topicId: "cvs", title: "Statin Mechanism", description: "HMG-CoA reductase inhibition and the compensatory rise in hepatic LDL receptor expression.", pearl: "Myopathy/rhabdomyolysis risk rises when statins are combined with fibrates or CYP3A4 inhibitors, which raise statin plasma levels.", Component: StatinDiagram },
  { topicId: "cvs", title: "PDE5 Inhibitor Mechanism", description: "The NO-cGMP pathway in corpus cavernosum smooth muscle — the same second messenger nitrates use, in a different tissue.", pearl: "Never combine a PDE5 inhibitor with nitrates — both raise cGMP through different mechanisms, risking severe, potentially fatal hypotension.", Component: Pde5InhibitorDiagram },
  { topicId: "cvs", title: "Heparin-Induced Thrombocytopenia (HIT)", description: "How PF4-heparin antibody complexes paradoxically activate platelets, causing a low platelet count with a HIGH clot risk.", pearl: "Never just switch a suspected HIT patient to LMWH — it cross-reacts. Switch to a non-heparin anticoagulant like argatroban instead.", Component: HitDiagram },
  ...
  { topicId: "cns", title: "Parkinson's Disease: Dopamine Pathway", description: "The nigrostriatal dopamine deficit, with levodopa/carbidopa, dopamine agonist, and MAO-B inhibitor sites of action marked.", pearl: "Carbidopa itself doesn't treat Parkinson's — it blocks peripheral levodopa breakdown so more reaches the brain, and reduces the nausea peripheral dopamine would otherwise cause.", Component: ParkinsonDiagram },
  { topicId: "cns", title: "Migraine Pharmacology & Triptans", description: "Trigeminovascular activation and CGRP release, with triptan (5-HT1B/1D agonist) sites of action marked.", pearl: "Triptans are contraindicated in coronary artery disease — the same vasoconstriction that relieves migraine can worsen cardiac ischemia.", Component: MigraineDiagram },
  { topicId: "cns", title: "Skeletal Muscle Relaxants: Baclofen vs Diazepam", description: "Baclofen's GABA-B spinal inhibition compared with diazepam's GABA-A action for treating spasticity.", pearl: "Baclofen carries far less sedation and dependence risk than diazepam at typical doses — a key reason it's often preferred for chronic spasticity.", Component: MuscleRelaxantDiagram },
  { topicId: "cns", title: "Anxiolytics: Buspirone vs Benzodiazepine", description: "Buspirone's 5-HT1A partial agonism compared with benzodiazepine GABA-A modulation.", pearl: "Buspirone's slow onset (2-4 weeks) rules it out for acute anxiety — but its lack of sedation and dependence risk makes it well suited to chronic, generalized anxiety.", Component: AnxiolyticComparisonDiagram },
  ...
  { topicId: "analgesics", title: "Opioid Mu-Receptor Signaling Cascade", description: "The full GPCR signaling chain — Gi protein, decreased cAMP, and the downstream ion effects that together produce analgesia.", pearl: "The same Gi-cAMP cascade explains why abrupt opioid withdrawal causes a rebound surge in cAMP and neurotransmitter release — the physiological basis of withdrawal symptoms.", Component: OpioidSignalingDiagram },
  ...
  { topicId: "analgesics", title: "Local Anesthetic Na+ Channel Block", description: "Use-dependent block in a peripheral nerve axon — the same channel states antiepileptics target, but with the added twist of differential fiber blockade.", pearl: "Pain and temperature sensation are lost well before motor function during a local anesthetic block — a direct consequence of small, rapidly-firing fibers being blocked before large motor fibers.", Component: LocalAnestheticBlockDiagram },
  { topicId: "analgesics", title: "Opioid Tolerance & Dependence", description: "Receptor desensitization and downregulation with chronic use, and the cAMP rebound that drives withdrawal.", pearl: "Tolerance to analgesia develops faster than tolerance to respiratory depression — a key reason escalating opioid doses can become dangerous.", Component: OpioidToleranceDiagram },
  { topicId: "analgesics", title: "Gout Pharmacology", description: "Xanthine oxidase inhibition vs colchicine's neutrophil block vs uricosuric drugs — three different strategies against gout.", pearl: "Allopurinol is never started during an acute attack — mobilizing urate stores can actually worsen the flare.", Component: GoutDiagram },
  ...
  { topicId: "antimicrobial", title: "Beta-Lactam Mechanism & Resistance", description: "How beta-lactams inhibit PBP-mediated transpeptidation, and how bacterial beta-lactamase defeats them — plus where beta-lactamase inhibitors intervene.", pearl: "A beta-lactamase inhibitor has no antibacterial activity of its own — it works purely by protecting its paired beta-lactam, which is why they're always given in combination (e.g. amoxicillin-clavulanate), never alone.", Component: BetaLactamDiagram },
  { topicId: "antimicrobial", title: "Antitubercular Drug Targets", description: "Isoniazid, rifampin, ethambutol, and pyrazinamide, each marked at their target on the mycobacterial cell.", pearl: "Four drugs hitting four independent targets simultaneously is exactly what prevents resistance from emerging — which is why TB is never treated with a single drug.", Component: AntitubercularDiagram },
  ...
  { topicId: "antiviral", title: "Antifungal Mechanisms", description: "Azoles, polyenes, and echinocandins, each marked at their independent target on the fungal cell.", pearl: "Selective toxicity comes from targeting ergosterol and fungal-wall components that human cells — which use cholesterol, not ergosterol — simply don't have.", Component: AntifungalDiagram },
  { topicId: "antiviral", title: "Antimalarial Mechanisms by Life-Cycle Stage", description: "Chloroquine, artemisinin, and primaquine mapped to the specific parasite stage each one targets.", pearl: "Chloroquine and artemisinin treat the acute illness; only primaquine reaches the dormant liver hypnozoites — screen for G6PD deficiency before giving it.", Component: AntimalarialDiagram },
  { topicId: "antiviral", title: "Anthelmintic Mechanisms", description: "Albendazole's microtubule inhibition vs ivermectin's glutamate-gated chloride channel activation.", pearl: "Glutamate-gated chloride channels don't exist in mammals — ivermectin's selective toxicity depends on this, plus its limited entry into the mammalian CNS via P-glycoprotein.", Component: AnthelminticDiagram },
  { topicId: "antiviral", title: "Acyclovir Nucleoside Analog Mechanism", description: "Activation by viral thymidine kinase and DNA chain termination — the selective-toxicity story behind acyclovir.", pearl: "Needing a viral (not human) enzyme for the first phosphorylation step is exactly why acyclovir has such a wide safety margin in uninfected cells.", Component: AcyclovirMechanismDiagram },
  { topicId: "antiviral", title: "Neuraminidase Inhibitor Mechanism", description: "How oseltamivir/zanamivir block influenza viral release from infected cells.", pearl: "Neuraminidase inhibitors are most effective when started within 48 hours of symptom onset — late initiation offers little benefit.", Component: NeuraminidaseInhibitorDiagram },
  ...
  { topicId: "endocrine", title: "Uterine Muscle: Oxytocin vs Tocolytics", description: "The same myometrium, opposite pharmacology — oxytocin driving contraction vs tocolytics producing relaxation.", pearl: "Nifedipine (a calcium channel blocker) and beta-2 agonists both relax the uterus but by different mechanisms — blocking Ca2+ entry vs raising cAMP.", Component: UterineMuscleDiagram },
  ...
  { topicId: "gi-resp", title: "Histamine H1 Allergic Response", description: "IgE-mediated mast cell degranulation and histamine release, with antihistamine site of action marked.", pearl: "First-generation antihistamines (diphenhydramine) cross the blood-brain barrier and sedate; second-generation agents (cetirizine, loratadine) largely don't.", Component: HistamineAllergyDiagram },
  { topicId: "gi-resp", title: "Antiemetic Mechanisms", description: "5-HT3 antagonists and D2 antagonists acting at the chemoreceptor trigger zone.", pearl: "Ondansetron can prolong the QT interval — worth checking other QT-prolonging drugs before adding it.", Component: AntiemeticDiagram },
  { topicId: "gi-resp", title: "Laxative Mechanisms Compared", description: "Bulk-forming, osmotic, and stimulant laxatives, each acting differently in the colon.", pearl: "Bulk-forming agents need adequate fluid intake to work — without it, they can worsen obstruction instead of relieving constipation.", Component: LaxativeDiagram },
  { topicId: "gi-resp", title: "Antitussive Mechanism", description: "Dextromethorphan/codeine acting on the medullary cough center.", pearl: "Dextromethorphan has minimal analgesic/addictive potential — codeine, a weak opioid, carries more of both.", Component: AntitussiveDiagram },
  { topicId: "gi-resp", title: "Iron Absorption & Anemia Pharmacology", description: "Duodenal ferroportin/hepcidin regulation and oral iron therapy.", pearl: "Take oral iron on an empty stomach or with vitamin C for better absorption — avoid taking it with antacids or dairy.", Component: IronAbsorptionDiagram },
  { topicId: "gi-resp", title: "Vitamin B12/Folate & Megaloblastic Anemia", description: "The methylmalonyl-CoA and homocysteine pathways behind megaloblastic anemia.", pearl: "Giving folate alone in B12 deficiency corrects the anemia but MASKS progression of neurologic damage — a classic exam trap.", Component: B12FolateDiagram },
  ...
  { topicId: "chemo", title: "Cancer Immunotherapy: Checkpoint Blockade", description: "PD-1/PD-L1 and CTLA-4 inhibition restoring T-cell activity against tumor cells.", pearl: "The trade-off for releasing the brakes on T-cells: immune-related adverse effects like colitis and pneumonitis, since healthy tissue can get caught in the response too.", Component: CheckpointBlockadeDiagram },
  { topicId: "chemo", title: "Calcineurin Inhibitor Immunosuppression", description: "How cyclosporine and tacrolimus block IL-2 gene transcription in T cells.", pearl: "Nephrotoxicity is the major dose-limiting toxicity for both cyclosporine and tacrolimus — levels are monitored closely.", Component: CalcineurinInhibitorDiagram },
  { topicId: "toxicology", title: "Alcohol Metabolism & Disulfiram", description: "The ADH → ALDH sequence, with disulfiram's block at ALDH causing acetaldehyde accumulation.", pearl: "This is the pharmacological basis of disulfiram aversive therapy — drinking while on it causes an immediate, unpleasant reaction (flushing, tachycardia, nausea).", Component: AlcoholMetabolismDiagram },
  { topicId: "toxicology", title: "Organophosphate Poisoning & Antidotes", description: "AChE inhibition and ACh accumulation, with atropine and pralidoxime marked at their distinct sites of action.", pearl: "Atropine only fixes the muscarinic symptoms — pralidoxime is essential for the nicotinic (motor) effects, and it must be given before the enzyme-OP bond 'ages' and becomes irreversible.", Component: OrganophosphateDiagram },
  { topicId: "toxicology", title: "Naloxone: Opioid Overdose Reversal", description: "Naloxone's competitive displacement of opioid from the mu-receptor in overdose.", pearl: "Naloxone's short half-life means re-sedation is possible after reversing a longer-acting opioid — repeat dosing or an infusion may be needed.", Component: NaloxoneReversalDiagram },
  { topicId: "toxicology", title: "Serotonin Syndrome Mechanism", description: "How combining serotonergic drugs (SSRI + MAOI or + tramadol) leads to excess synaptic 5-HT.", pearl: "Clonus and hyperreflexia (too much serotonin) distinguish serotonin syndrome from neuroleptic malignant syndrome, which features rigidity instead.", Component: SerotoninSyndromeDiagram },
];
